                ...
                <form action="{{ route('admin.subjects.update', $subject) }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select name="units" required>
                            ...
                            <option value="1" {{ old('units', $subject->units) == 1 ? 'selected' : '' }}>1 Unit</option>
                            <option value="2" {{ old('units', $subject->units) == 2 ? 'selected' : '' }}>2 Units</option>
                            <option value="3" {{ old('units', $subject->units) == 3 ? 'selected' : '' }}>3 Units</option>
                            <option value="4" {{ old('units', $subject->units) == 4 ? 'selected' : '' }}>4 Units</option>
                            <option value="5" {{ old('units', $subject->units) == 5 ? 'selected' : '' }}>5 Units</option>
                            <option value="6" {{ old('units', $subject->units) == 6 ? 'selected' : '' }}>6 Units</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label><i class="fas fa-align-left"></i> Description</label>
                        <textarea name="description" rows="4">{{ old('description', $subject->description) }}</textarea>
                    </div>

                    <div class="form-actions">
                        <form action="{{ route('admin.subjects.destroy', $subject) }}" method="POST" class="delete-form" onsubmit="return confirm('Are you sure you want to delete this subject?')">
                            @csrf
                            @method('DELETE')
